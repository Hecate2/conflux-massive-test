...
def pick_zone(c: EcsClient, r: str) -> str:
    ...
    if not zones: raise RuntimeError(f"no zones in {r}")
...
def ensure_vswitch(c: EcsClient, r: str, vpc: str, zone: str, name: str, cidr: str, vpc_cidr: str) -> str:
    ...
    for v in vsws:
        if v.v_switch_name == name and v.zone_id == zone: return v.v_switch_id
...
def ensure_sg(c: EcsClient, r: str, vpc: str, name: str) -> str:
    ...
    for g in resp.body.security_groups.security_group or []:
        if g.security_group_name == name: return g.security_group_id
...
def auth_port(c: EcsClient, r: str, sg: str, port: int) -> None:
    ...
    for p in resp.body.permissions.permission or []:
        if p.ip_protocol == "tcp" and p.port_range == f"{port}/{port}" and p.source_cidr_ip == "0.0.0.0/0": return
...
def pick_instance_type(c: EcsClient, cfg: EcsConfig) -> Optional[tuple[str, str, Optional[str]]]:
    ...
    for z in resp.body.available_zones.available_zone or []:
        if z.status_category not in {"WithStock", "ClosedWithStock"}: continue
        types = [i.value for r in (z.available_resources.available_resource or []) if r.type == "InstanceType" for i in (r.supported_resources.supported_resource or []) if i.status_category in {"WithStock", "ClosedWithStock"}]
        ...
        cands = [t for t in tmap.values() if t.cpu_core_count == cfg.min_cpu_cores and t.memory_size and cfg.min_memory_gb <= t.memory_size <= cfg.max_memory_gb and (not cfg.cpu_vendor or cfg.cpu_vendor.lower() in (t.physical_processor_model or "").lower())]
        if cands:
...
def create_instance(c: EcsClient, cfg: EcsConfig, disk_size: int = 40) -> str:
    ...
    req = ecs_models.CreateInstanceRequest(region_id=cfg.region_id, zone_id=cfg.zone_id, image_id=img, instance_type=cfg.instance_type, security_group_id=cfg.security_group_id, v_switch_id=cfg.v_switch_id, key_pair_name=cfg.key_pair_name, instance_name=name, internet_max_bandwidth_out=cfg.internet_max_bandwidth_out, internet_charge_type="PayByTraffic", instance_charge_type="PostPaid", spot_strategy=cfg.spot_strategy if cfg.use_spot else None, system_disk=disk)
    resp = c.create_instance(req)
    if not resp.body or not resp.body.instance_id: raise RuntimeError("instance creation failed")
...
def ensure_keypair(c: EcsClient, r: str, name: str, key_path: str) -> None:
    ...
    if resp.body.key_pairs.key_pair: return
...
async def wait_ssh(host: str, user: str, key: str, timeout: int, interval: int = 3) -> None:
    ...
    while time.time() < deadline:
        try:
            ...
            conn.close(); await conn.wait_closed(); return
        except Exception: await asyncio.sleep(interval)
    raise TimeoutError(f"SSH not ready for {host}")


# --- Image ---
def _img_name(prefix: str, ref: str) -> str: return f"{prefix}-conflux-{ref.replace('/', '-').replace(':', '-')}"
...
def find_ubuntu(c: EcsClient, r: str) -> str:
    ...
    if not imgs: raise RuntimeError("no Ubuntu 20.04 image")
...
def find_img(c: EcsClient, r: str, name: str) -> Optional[str]:
    ...
    for i in resp.body.images.image or []:
        if i.image_name == name: return i.image_id
    return None
...
def wait_img(c: EcsClient, r: str, img: str, poll: int, timeout: int) -> None:
    def chk():
        ...
        if st in {"CreateFailed", "UnAvailable", "Deprecated"}: raise RuntimeError(f"image failed: {st}")
        return st == "Available"
    wait_until(chk, timeout=timeout, retry_interval=poll)
...
async def default_prepare(host: str, cfg: EcsConfig) -> None:
    ...
    async with asyncssh.connect(host, username=cfg.ssh_username, client_keys=[key], known_hosts=None) as conn:
        async def run(cmd: str) -> None:
            ...
            if r.stderr: logger.warning(r.stderr.strip())
            if r.exit_status != 0: raise RuntimeError(f"failed: {cmd}")
        ...
        await run(f"sudo bash -lc 'cd /opt/conflux/src/conflux-rust; git fetch --depth 1 origin {cfg.conflux_git_ref} || true; git checkout {cfg.conflux_git_ref} || git checkout FETCH_HEAD; git submodule update --init --recursive; curl https://sh.rustup.rs -sSf | sh -s -- -y; source $HOME/.cargo/env; cargo build --release --bin conflux; install -m 0755 target/release/conflux /usr/local/bin/conflux'")


def create_server_image(cfg: EcsConfig, dry_run: bool = False, prepare_fn: Callable[[str, EcsConfig], Coroutine[Any, Any, None]] = default_prepare) -> str:
    name = _img_name(cfg.image_prefix, cfg.conflux_git_ref)
    c = client(cfg.credentials, cfg.region_id, cfg.endpoint)
    if not cfg.base_image_id: cfg.base_image_id = find_ubuntu(c, cfg.region_id)
    ...
    async def prepare(host: str, cfg: EcsConfig) -> None:
        ...
        async with asyncssh.connect(host, username=cfg.ssh_username, client_keys=[key], known_hosts=None) as conn:
            async def run(cmd: str, check: bool = True) -> None:
                ...
                if r.stdout: logger.info(r.stdout.strip())
                if r.stderr: logger.warning(r.stderr.strip())
                if check and r.exit_status != 0: raise RuntimeError(f"failed: {cmd}")
            await run("sudo apt-get update -y")
            ...
            await asyncssh.scp(str(ca), (conn, f"/tmp/{ca.name}")); ca.unlink(missing_ok=True)
...
def add_args(p: argparse.ArgumentParser) -> None:
    p.add_argument("--conflux-git-ref", default="v3.0.2"); p.add_argument("--base-image-id", default=None)
    p.add_argument("--instance-type", default=None); p.add_argument("--min-cpu-cores", type=int, default=4)
    p.add_argument("--min-memory-gb", type=float, default=8.0); p.add_argument("--max-memory-gb", type=float, default=8.0)
    p.add_argument("--v-switch-id", default=None); p.add_argument("--security-group-id", default=None)
    p.add_argument("--key-pair-name", default=DEFAULT_KEYPAIR); p.add_argument("--region-id", default=DEFAULT_REGION)
    p.add_argument("--zone-id", default=None); p.add_argument("--endpoint", default=None)
    ...
    p.add_argument("--no-cleanup", action="store_true"); p.add_argument("--dry-run", action="store_true")


def cfg_from_args(a) -> EcsConfig:
    return EcsConfig(credentials=load_credentials(), region_id=a.region_id, zone_id=a.zone_id, endpoint=a.endpoint or load_endpoint(), base_image_id=a.base_image_id, instance_type=a.instance_type, min_cpu_cores=a.min_cpu_cores, min_memory_gb=a.min_memory_gb, max_memory_gb=a.max_memory_gb, use_spot=a.spot, spot_strategy=a.spot_strategy, v_switch_id=a.v_switch_id, security_group_id=a.security_group_id, vpc_name=a.vpc_name, vswitch_name=a.vswitch_name, security_group_name=a.security_group_name, vpc_cidr=a.vpc_cidr, vswitch_cidr=a.vswitch_cidr, key_pair_name=a.key_pair_name, ssh_username=a.ssh_username, ssh_private_key_path=a.ssh_private_key, conflux_git_ref=a.conflux_git_ref, image_prefix=a.image_prefix, internet_max_bandwidth_out=a.internet_max_bandwidth_out, search_all_regions=a.search_all_regions, cleanup_builder_instance=not a.no_cleanup, poll_interval=a.poll_interval, wait_timeout=a.wait_timeout)


def main() -> None:
    p = argparse.ArgumentParser(description="Create Alibaba Cloud image for Conflux"); add_args(p); a = p.parse_args()
    logger.info(f"image: {create_server_image(cfg_from_args(a), dry_run=a.dry_run)}")
