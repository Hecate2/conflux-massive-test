...
def create_instances_in_zone(
    ...
    def _create_single(single_max: int, single_min: int) -> Tuple[list[str], CreateInstanceError]:
        ...
        if needed_min_for_fallback > 0:
            ...
            req_nospot.min_amount = request_min
        ...
        if len(total_ids) > 0:
            if len(total_ids) < single_max:
                logger.warning(f"Partially created instances at {region_info.id}/{zone_info.id}: requested={single_max}, actual={len(total_ids)}")
